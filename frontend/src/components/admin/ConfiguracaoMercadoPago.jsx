...
const salvar = () => {
  axios.post("/api/empresa-config/mercadopago", form, { headers })
    .then(() => showSuccess("Configurações salvas com sucesso!"))
    .catch(() => showError("Erro ao salvar configurações."));
};

const testarConexao = () => {
  axios.post("/api/empresa-config/mercadopago/testar", {}, { headers })
    .then(res => {
      showSuccess("Comunicação OK com Mercado Pago!\nUsuário: " + res.data.usuario.nickname, { title: "Teste de conexão" });
    })
    .catch(err => {
      console.error(err);
      showError("Falha na comunicação com Mercado Pago. Verifique o Access Token.");
    });
};
