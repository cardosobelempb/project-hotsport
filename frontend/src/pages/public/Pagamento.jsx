...
export default function Pagamento() {
  ...
  return (
    <PublicBanners
      ...
      contentClassName="flex flex-col items-center px-4 py-10"
    >
      ...
      {plano && (
        <div className="mb-6 text-center">
          ...
          <p className="text-gray-600">
            {Number(plano.valor / 100).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}
          </p>
        </div>
      )}
      ...
      {metodo === null && plano && (
        <div className="w-full max-w-md space-y-4">
          ...
          <button
            ...
            className="w-full text-white py-3.5 rounded-lg font-semibold shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200 flex items-center justify-center gap-2 disabled:opacity-50 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700"
          >
            Voltar
          </button>
        </div>
              ...
              <div className="w-full max-w-md mb-4">
                ...
                {trialErro && (
                  <Alert variant="error" className="mb-3">
                    <span><strong>Nao foi possivel liberar acesso free:</strong> {trialErro}</span>
                  </Alert>
                )}
      ...
      {metodo === "cartao" && cartaoStatus !== "pending" && fingerprintStatus === "ready" && (
        <form onSubmit={submeterCartao} className="w-full max-w-md space-y-3">
          ...
          >
            {cartaoLoading ? "Processando..." : (plano ? `Pagar ${Number(plano.valor / 100).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}` : "Pagar")}
          </button>
