...
export default function CadastroLGPD() {
    ...
    try {
      ...
      if (data.gateway && data.username) {
        ...
        if (data.oferece_planos) {
          // Portal oferece plano pago: mostra a tela de escolha em vez de conectar direto
  ...
  if (sucesso) {
    return (
      <div
        className={`min-h-screen flex items-center justify-center text-white px-4 ${!bgStyle ? 'bg-gradient-to-br from-sky-800 via-blue-900 to-blue-950' : ''}`}
        style={bgStyle}
      >
        <div className="text-center max-w-sm">
          <div className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 ${conectando ? 'bg-green-500/20' : 'bg-blue-500/20'}`}>
            {conectando ? (
  ...
  return (
    <PublicBanners
      ...
      className={`text-white ${!bgStyle ? 'bg-gradient-to-br from-sky-800 via-blue-900 to-blue-950' : ''}`}
      style={bgStyle}
