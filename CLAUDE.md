...
- **Icones:** Lucide React 0.525.0, Radix UI Icons
- **HTTP:** Axios 1.10.0
- **Masks:** react-input-mask 2.0.4

### Infraestrutura
- **VPN:** WireGuard (wg-easy via Docker)
- **RADIUS:** FreeRADIUS 3.0 (tabelas no mesmo MySQL)
- **Containers:** Docker Compose (docker-compose.yml / .dev.yml / .prod.yml)
- **Web Server:** Nginx
- **Process Manager:** PM2
...
│   ├── migrations/                  # Migracoes incrementais (001-015)
│   ├── jobs/
│   │   ├── estrutura.sql            # Schema completo do banco (dump)
│   │   ├── estrutura.sql.bak        # Backup schema manual antigo
│   │   └── verificaExpiracoes.js   # Verificar planos expirados
│   ├── routes/                      # Rotas legadas admin
│   └── src/
│       ├── controllers/             # Logica de negocio (33 controllers)
│       ├── routes/                  # Rotas da API (32 arquivos)
│       ├── models/                  # Modelos Sequelize (5 models)
│       ├── services/
│       │   └── whatsappNotify.js    # Service notificacoes WhatsApp
│       ├── constants/
│       │   └── whatsappDefaults.js  # Templates e defaults WhatsApp/Portal
│       ├── views/
│       │   └── emergency.html       # Pagina de emergencia
│       ├── middleware/
│       │   ├── auth.js              # Verificacao JWT
│       │   ├── tenant.js            # Resolucao empresa_id
│       │   ├── authorize.js         # Verificacao de roles
│       │   ├── checkPermissao.js    # Verificacao de permissoes granulares
│       │   └── uploadCampanha.js    # Multer upload para campanhas
│       ├── jobs/
│       │   └── syncConnectionLogs.js # Sync radacct -> connection_logs
│       └── utils/
│           ├── mikrotikClient.js    # Cliente RouterOS API
│           ├── hotspotSetup.js      # Configuracao hotspot automatizada
│           └── leadUtils.js         # Utilitarios de leads
├── frontend/
│   ├── vite.config.js
│   ├── tailwind.config.js
│   ├── public/                      # Assets estaticos (mp-security.js, logos)
│   ├── dist/                        # Build de producao
│   └── src/
│       ├── App.jsx                  # Rotas da aplicacao
│       ├── main.jsx                 # Entry point React
│       ├── contexts/
│       │   └── AuthContext.jsx      # Contexto de autenticacao global
│       ├── components/
│       │   └── admin/               # Componentes admin reutilizaveis (5)
│       ├── utils/
│       │   ├── cpfUtils.js          # Validacao/formatacao CPF
│       │   └── hotspotRedirect.js   # Redirecionamento hotspot
│       └── pages/
│           ├── admin/               # Paginas painel admin (21 paginas)
│           ├── public/              # Paginas captive portal (10 paginas)
│           └── super/               # Paginas super admin (5 paginas)
├── infra/
│   └── wireguard/
│       └── docker-compose.yml       # WireGuard wg-easy container
├── docker/                          # Configuracoes Docker adicionais
├── freeradius_conf/                 # Configuracoes FreeRADIUS
├── portainer/                       # Portainer container management
├── dist-images/                     # Imagens de distribuicao
├── docker-compose.yml               # Orquestracao principal
├── docker-compose.dev.yml           # Orquestracao desenvolvimento
├── docker-compose.prod.yml          # Orquestracao producao
└── docs/
    └── MIDDLEWARE-SYSTEM.md         # Documentacao sistema de middleware
```
...
| `grupos_permissao` | Grupos de permissao RBAC (empresa_id, nome, permissoes JSON) |

#### Rede / Hotspot
| Tabela | Descricao |
|--------|-----------|
| `mikrotiks` | Roteadores MikroTik (empresa_id, nome, ip, usuario, senha, porta, vpn_ip, portal_id, status) |
| `nas` | Clientes RADIUS NAS (empresa_id, nasname, shortname, secret) |
| `portais` | Portais captive (empresa_id, mikrotik_id, tipo, html_content, template_id, custom_css, logo_url, cores, campos_cadastro, mostrar_planos, mostrar_lgpd, url_redirect, whatsapp_enabled, whatsapp_template, configuracoes JSON) |
| `portal_templates` | Templates reutilizaveis (nome, html_template, css_template, tipo: basico/planos/lgpd/completo/lead-passivo) |
...
| `pagamentos` | Transacoes (empresa_id, plano_id, valor, status, mp_pagamento_id, mac, cpf, IP, portal_id, telefone, trial_liberado_em) |
...
| `connection_logs_sync` | Controle de sincronizacao (last_synced_radacctid) |

#### Campanhas
| Tabela | Descricao |
|--------|-----------|
| `campanhas` | Campanhas de marketing (empresa_id, nome, tipo, conteudo, ativo, datas) |

#### WhatsApp
| Tabela | Descricao |
|--------|-----------|
| `whatsapp_logs` | Log de envios WhatsApp (empresa_id, portal_id, telefone, status: ok/erro/skipped, mensagem) |

#### Sistema de Atualizacao (master)
| Tabela | Descricao |
|--------|-----------|
| `updates` | Pacotes de atualizacao publicados (versao, descricao, criado_em) |
| `update_files` | Arquivos incluidos em cada update (update_id, path, conteudo, md5) |
| `update_migrations` | SQL migrations de cada update (update_id, sql) |
| `file_snapshots` | Baseline MD5 de arquivos para deteccao de mudancas |
| `schema_snapshots` | Baseline JSON de colunas/indices para deteccao DDL |

#### Sistema de Atualizacao (aluno)
| Tabela | Descricao |
|--------|-----------|
| `applied_updates` | Ultimo update aplicado no servidor aluno |
| `system_backups` | Backups realizados antes de cada update |
| `update_apply_logs` | Timeline de etapas de cada apply (step, status, message) |
...
Executar em ordem (o install.sh executa automaticamente todas `[0-9][0-9][0-9]_*.js`):
```bash
cd /var/www/hotspot/backend
node migrations/001_multi_tenant.js         # Cria empresas, empresa_configs, empresa_id em todas tabelas
...
node migrations/006_mikrotik_portal_id.js   # Adiciona portal_id na tabela mikrotiks
node migrations/007_portal_template_lead_passivo.js  # Template lead passivo
node migrations/008_inserir_portais_lead_passivo.js  # Portais lead passivo iniciais
node migrations/009_system_updates.js       # Cria applied_updates e system_backups
node migrations/010_update_publish.js       # Cria updates, update_files, update_migrations, file_snapshots
node migrations/011_campanhas.js            # Cria tabela campanhas
node migrations/012_whatsapp_notif.js       # Adiciona whatsapp_enabled/template em portais, cria whatsapp_logs
node migrations/013_pix_trial.js            # Adiciona trial_liberado_em em pagamentos
node migrations/014_schema_snapshots.js     # Cria schema_snapshots (deteccao DDL)
node migrations/015_update_apply_logs.js    # Cria update_apply_logs (timeline do apply)
```
...
### Middleware Chain
```
request -> auth.js (verifica JWT) -> tenant.js (resolve empresa_id) -> authorize(roles) -> [checkPermissao()] -> controller
```

O middleware `tenant.js` extrai `empresa_id` do JWT. Super admin pode sobrescrever via header `x-empresa-id`. O `checkPermissao` e' opcional e usado em rotas com RBAC granular por grupos.
...
POST   /api/pagamentos/gerar-cartao            # Gerar pagamento cartao de credito (server-side)
POST   /api/pagamentos/pix-trial               # Liberar acesso temporario ao copiar PIX
POST   /api/pagamentos/mp-device-session/:tipo # Proxy device fingerprint Mercado Pago
...
POST   /api/lead/login                         # Login via captacao de lead
POST   /api/lead/passivo                       # Captacao passiva de lead
POST   /api/auth/temp-access                   # Acesso temporario
GET    /api/planos-publicos                    # Listar planos (publico)
GET    /api/planos-publicos/:id                # Plano especifico com config portal
POST   /api/registro                           # Registro nova empresa
GET    /api/campanhas/publicas/:mikrotikId     # Campanhas ativas do MikroTik
POST   /api/updates/check                      # Verificar updates disponiveis (valida email Hotmart)
GET    /api/updates/download/:id               # Download pacote de update
```
...
POST                  /api/mikrotiks/:id/enviar-hotspot  # Wizard auto-configuracao

# Portais
GET/POST/PUT/DELETE   /api/portais             # CRUD portais captive
GET/POST              /api/portal-templates     # Templates de portal
POST                  /api/portais/:id/whatsapp-preview  # Preview template WhatsApp
POST                  /api/portais/:id/whatsapp-teste    # Teste envio WhatsApp real
...
GET                   /api/radius-logs                # Logs accounting com filtros
GET                   /api/radius-logs/export         # Exportar CSV compliance
...
GET                   /api/whatsapp/logs        # Listar logs de envios
DELETE                /api/whatsapp/logs        # Limpar logs antigos
...
# Campanhas
GET/POST/PUT/DELETE   /api/campanhas            # CRUD campanhas de marketing
GET                   /api/campanhas/publicas   # Campanhas ativas (publico)

# Admin Users
GET/POST/PUT/DELETE   /api/admins              # Gerenciar admins

# Grupos de Permissao
GET/POST/PUT/DELETE   /api/grupos-permissao    # CRUD grupos de permissao RBAC

# Limpeza
GET/POST              /api/limpeza             # Jobs de limpeza de dados

# Login Portal (captive)
POST                  /api/login-portal        # Login via portal customizado

# Backups
GET                   /api/system-backup       # Listar backups
POST                  /api/system-backup       # Criar backup
GET                   /api/system-backup/:id/download # Download backup

# Sistema de Atualizacao (master)
POST                  /api/update-publish/snapshot      # Tirar snapshot de schema/arquivos
POST                  /api/update-publish/detect        # Detectar alteracoes
POST                  /api/update-publish/publish       # Publicar update
GET                   /api/update-publish/updates       # Listar updates publicados

# Sistema de Atualizacao (aluno)
POST                  /api/updates/check               # Verificar updates disponiveis
POST                  /api/system-update/apply         # Aplicar update
GET                   /api/system-update/logs          # Ver logs do apply
...
| `/pagamento/:id` | Pagamento | Pagamento MP PIX ou Cartao |
| `/cadastro-cliente` | CadastroCliente | Cadastro cliente para planos |
| `/lgpd` | LgpdAuto | Consentimento LGPD automatico |
| `/cadastro` | CadastroLGPD | Formulario cadastro LGPD |
| `/cadastro-lead` | CadastroLead | Formulario captacao de lead |
| `/lead-passivo` | CadastroLeadPassivo | Captura passiva de lead |
| `/campanha/:id` | CampanhaPlayer | Player de campanha de marketing |
| `/hotspot-login` | LoginHotspot | Login hotspot/captive portal |
...
| `/admin/:slug/whatsapp` | WhatsApp | Historico de envios WhatsApp |
| `/admin/:slug/campanhas` | Campanhas | Gerenciar campanhas de marketing |
| `/admin/:slug/campanhas/:id/editor` | CampanhaEditor | Editor visual de campanha |
| `/admin/:slug/grupos-permissao` | GruposPermissao | Grupos de permissao RBAC |
| `/admin/:slug/empresas` | EmpresasAdmin | Gerenciar empresas (owner) |

### Super Admin
| Rota | Componente | Descricao |
|------|-----------|-----------|
| `/super` | SuperDashboard | Visao geral do sistema |
| `/super/empresas` | Empresas | Gerenciar todas as empresas |
| `/super/atualizar` | AtualizarSistema | Aplicar updates do sistema (aluno) |
| `/super/publicar` | PublicarAtualizacao | Publicar updates (master) |
| `/super/backups` | Backups | Gerenciar backups do sistema |
...
**radiusController.js** - `backend/src/controllers/radiusController.js`
...
**radiusLogsController.js** - `backend/src/controllers/radiusLogsController.js`
...
| Expiracoes | `jobs/verificaExpiracoes.js` | Verifica planos expirados e remove do MikroTik + RADIUS |

**Execucao:** Manual via `node <arquivo>.js` ou via node-cron integrado ao server.js.
...
#### Kit de UI admin (`frontend/src/components/ui/`) — padroes obrigatorios
- Paginas admin usam os componentes do kit (`Button, IconButton, Input, Textarea, Select, Card, Table, Badge, StatusToggle, Pagination`) — nunca reimplementar botao/tabela/badge com classes cruas.
- **Acoes de linha de tabela: SEMPRE icone-only** via `IconButton` (suporta `loading`) com `title` obrigatorio. Icones semanticos lucide-react: Adicionar=`Plus`, Editar=`Pencil`, Excluir=`Trash2`, Visualizar=`Eye`, Exportar=`Download`, Testar conexao=`Plug`, Info=`Info`, Wizard/Hotspot=`Wifi`, Enviar=`Send`, Diagnostico=`Activity`, Desconectar=`WifiOff`.
- **Coluna Status com liga/desliga: SEMPRE `StatusToggle`** (`frontend/src/components/ui/StatusToggle.jsx`) — Badge clicavel Ativo (info) / Inativo (neutral), alterna direto via PUT sem modal. Referencias: Planos.jsx e Campanhas.jsx.
- **Tabelas: sem scroll horizontal.** Maximo ~6 colunas visiveis; agrupar pares relacionados em celulas empilhadas (linha principal + linha `text-[11px] text-gray-500`), ex: Usuario/MAC, IP/NAS, Conectado/Desconectado. Colunas secundarias usam `hideOn="md"|"lg"`. Referencia: SessoesLog.jsx.
- Paginacao 12 itens/pagina em toda tabela (hook `usePagination` ou `per_page=12` no backend).
- O kit tambem exporta `Modal` (overlay base) e `Alert` (mensagem inline) — ver secao "Mensagens, erros e confirmacoes" abaixo.

#### Mensagens, erros e confirmacoes — padroes obrigatorios
- **PROIBIDO `alert()` e `confirm()` nativos** em qualquer pagina. Usar `useFeedback()`
  (`frontend/src/contexts/FeedbackContext.jsx`, provider global montado em `main.jsx`):
  - `showError(msg, {title?})` / `showSuccess(msg, {title?})` / `showInfo(msg, {title?})` — modal padrao
    dark com icone semantico; `msg` aceita string ou array de linhas (array vira lista de linhas no corpo).
  - `const ok = await confirm({ title, message, confirmText?, cancelText?, danger? })` — retorna
    `Promise<boolean>`. **SEMPRE usar com `await`** — sem await a Promise e truthy e o `if (!confirm(...))`
    nunca bloqueia. Acoes destrutivas (excluir/remover/desconectar): `danger: true` + `confirmText` com o
    verbo da acao ("Excluir", "Remover", "Desconectar").
- **Modais custom**: sempre construir sobre `Modal` do kit (`frontend/src/components/ui/Modal.jsx`, props:
  `open, onClose, title, icon, iconClassName, size, footer, closeOnBackdrop`) — nunca copiar o overlay
  `fixed inset-0 bg-black/70...` na mao.
- **Paginas publicas (captive portal)**: mensagens inline com `<Alert variant="error|success|warning|info">`
  (`frontend/src/components/ui/Alert.jsx`, tema claro) — nunca repetir classes `bg-red-50 border-red-200...`
  cruas. Nao usar o modal dark do FeedbackContext nas paginas publicas (o cliente final ve tema claro).
- **Toda mensagem exibida ao usuario e em PT-BR**, amigavel e acionavel (dizer o que aconteceu e o que fazer),
  sem stack trace, sem texto tecnico em ingles, sem emojis de status (✅/❌ — o icone do modal ja indica).
- **Backend — envelope de erro padrao (codigo novo): `{ error: "mensagem PT-BR" }`** com status HTTP correto.
  Error handler global em `backend/src/middleware/errorHandler.js` (registrado por ultimo no `server.js`);
  controller novo pode lancar `Object.assign(new Error("msg PT-BR"), { status: 4xx })` ou deixar o erro subir
  (Express 5 encaminha rejeicoes async automaticamente). O handler so expoe `err.message` quando `status < 500`.
- **NUNCA enviar `err.message`/`err.stack` de erro interno na resposta HTTP** (`error: err.message` e vazamento).
  Regra: mensagem amigavel no JSON + `console.error(err)` completo no log do servidor. Excecao: erros lancados
  intencionalmente com `status` definido e mensagem ja amigavel (padrao do radiusService).
- Codigo legado com `{ message: "..." }` nao precisa ser refatorado em massa — apenas nao criar codigo novo
  nesse formato.
...
Sistema de criacao e exibicao de campanhas por empresa, exibidas no captive portal antes ou depois do login.

### Arquivos-chave
- `backend/src/controllers/campanhasController.js` - CRUD campanhas (admin)
- `backend/src/controllers/campanhasPublicController.js` - campanhas ativas (publico)
- `backend/src/routes/campanhasRoutes.js` - rotas protegidas
- `backend/src/routes/campanhasPublicRoutes.js` - rotas publicas
- `frontend/src/pages/admin/Campanhas.jsx` - listagem admin
- `frontend/src/pages/admin/CampanhaEditor.jsx` - editor visual
- `frontend/src/pages/public/CampanhaPlayer.jsx` - exibicao publica

### Endpoints
```
GET/POST/PUT/DELETE   /api/campanhas                   # CRUD (protegido)
GET                   /api/campanhas/publicas/:mikrotikId  # Campanhas ativas (publico)
```

---

## Grupos de Permissao (RBAC Granular)

Sistema de permissoes granulares por grupo, complementar aos roles (owner/manager/operator). Permite definir quais modulos cada grupo de usuarios pode acessar.

### Arquivos-chave
- `backend/src/controllers/grupoPermissaoController.js` - CRUD grupos
- `backend/src/middleware/checkPermissao.js` - middleware de verificacao
- `backend/src/routes/grupoPermissaoRoutes.js` - rotas protegidas
- `frontend/src/pages/admin/GruposPermissao.jsx` - gerenciamento de grupos

---

## WireGuard VPN (wg-easy)

> **Doc tecnico completo:** `docs/WIREGUARD-VPN.md`

Container Docker `wg-easy:14` que gerencia peers WireGuard. Permite conectar MikroTiks atras de NAT ao servidor via tunel UDP. Backend autentica via `POST /api/session` e usa cookie de sessao para CRUD de peers.

### Arquitetura (resumo)

```
[MikroTik NAT] -- UDP (51820/13231) --> [wg-easy container]
                                              |
                                    HTTP interno (51821)
                                              |
                                    backend wireguardController.js
                                              |
                                    RouterOS API 8728 via 10.8.0.x
```
...
| Variavel | Proposito |
|---|---|
| `WG_HOST` | IP/dominio que aparece no config do peer (LAN em dev, IP publico em prod) |
| `WG_PASS` | Senha plain text usada pelo backend pra autenticar na API wg-easy |
| `PASSWORD_HASH` | Hash bcrypt da senha — em `wg-password.env` com `$$` escapados |

### Arquivos-chave

- `docker-compose.yml` — servico wg-easy, `env_file: wg-password.env`
- `wg-password.env` — `PASSWORD_HASH` com `$$` escapados (evita corrupcao Docker Compose)
- `backend/src/controllers/wireguardController.js` — auth, CRUD peers, script RouterOS
- `backend/scripts/generate-wg-hash.js` — helper para gerar hash bcrypt

### Gotchas gravados em sangue

1. **v14 rejeita `PASSWORD` — obrigatorio usar `PASSWORD_HASH`** — variavel `PASSWORD` faz o container abortar. Usar apenas `env_file` com `PASSWORD_HASH`.
2. **Docker Compose v2 corrompe hash bcrypt** — `$NOi3...` dentro do hash e tratado como variavel e substituido por string vazia. Fix: escapar com `$$` no `wg-password.env`.
3. **v15 tem formato de config diferente** — pin em `:14`, nao atualizar sem ler migracao.
...
10. **Sem sistema de filas:** Jobs rodam como scripts Node.js avulsos ou via node-cron, sem Bull/Redis/RabbitMQ.

11. **Migrations novas precisam de 3 digitos:** O loop do install.sh pega `[0-9][0-9][0-9]_*.js`. Ex: `016_minha_feature.js`.

12. **portal_id explicito > mikrotiks.portal_id:** Em fluxos com redirect entre portais, sempre preferir o `portal_id` explicito vindo da URL/body. Ver secao "Redirect entre Portais".

13. **Correcoes valem para dev E prod:** Qualquer correcao de bug deve ser aplicada nos tres arquivos Docker Compose (`docker-compose.yml`, `docker-compose.dev.yml`, `docker-compose.prod.yml`) quando relevante, e o codigo do backend/frontend e identico em ambos os ambientes — nao ha branches separados. Corrigir em um lugar ja corrige em ambos.

14. **NAS RADIUS usa vpn_ip quando disponivel:** O campo `nas.nasname` deve receber `mikrotiks.vpn_ip` (quando preenchido) em vez de `mikrotiks.ip`. MikroTiks atras de NAT comunicam com o FreeRADIUS via IP WireGuard — usar o IP local como nasname faz o FreeRADIUS rejeitar todos os pacotes RADIUS.

15. **Wizard empurra login.html E status.html:** O wizard de configuracao de hotspot (`/api/mikrotiks/:id/enviar-hotspot`) deve sempre tentar enviar os dois arquivos (`hotspot/login.html` e `hotspot/status.html`). Sem o status.html, o MikroTik exibe sua page padrao apos o login, sem o estilo/logo da empresa.

16. **Portais padrao criados junto com a empresa:** Tanto `registroController.criarEmpresa` quanto `empresaController.criarEmpresa` devem criar os 8 portais padrao (LGPD, Planos, Lead, Lead-Passivo, Acesso Wi-Fi, Trial por Tempo, Campanha+Acesso, Reconexao). Nao criar apenas na rota de super admin.

17. **Portal `reconexao` — exibido quando o tempo do usuario expira:** Tipo `reconexao`, URL `/reconectar`, serve como dashboard de renovacao. Busca o usuario cadastrado pelo MAC via `GET /api/reconexao/buscar-usuario?mac=X&empresa_id=Y` (tabela `leads`), exibe os planos pagos da empresa (filtro `valor > 0`) e redireciona para `/pagamento/:planId` com `cpf`, `cliente_id`, `mac`, `ip`, `mikrotik_id` e `portal_id` propagados via querystring. Apos o pagamento aprovado o usuario e reconectado usando seu CPF ja cadastrado. Backend: `backend/src/controllers/reconexaoController.js`, `backend/src/routes/reconexaoRoutes.js`. Frontend: `frontend/src/pages/public/PortalReconexao.jsx`. Seed: portal inserido em `estrutura.sql` e criado por `empresaController` e `registroController`. Para vincular ao MikroTik, o admin associa o portal do tipo `reconexao` ao perfil do hotspot — o redirect para `/reconectar?mac=...&empresa_id=...` deve ser configurado como destino quando o tempo expira.
