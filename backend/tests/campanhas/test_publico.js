...
(async () => {
  ...
  try {
    ...
    // 5. Vincula a campanha ao portal via portal_campanhas (task 012 — N:N)
    await db.execute(
      'INSERT INTO portal_campanhas (portal_id, campanha_id) VALUES (?, ?)',
      [portalId, campanhaId]
    );
    ...
    const r7 = await req('POST', `/api/public/campanha/${portalId}/view`, null, { campanha_id: campanhaId });
