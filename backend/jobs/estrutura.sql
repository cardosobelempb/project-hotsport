...
CREATE TABLE `banners` (
  ...
  `ad_client` varchar(30) DEFAULT NULL,
  `ad_slot` varchar(20) DEFAULT NULL,
  `ad_format` varchar(30) DEFAULT NULL,
  `ad_full_width` tinyint(1) DEFAULT '1',
  `ad_width` smallint DEFAULT NULL,
  `ad_height` smallint DEFAULT NULL,
  `ativo` tinyint(1) DEFAULT '1',
  `criado_em` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `atualizado_em` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `idx_banners_empresa` (`empresa_id`),
  CONSTRAINT `fk_banners_empresa` FOREIGN KEY (`empresa_id`) REFERENCES `empresas` (`id`) ON DELETE CASCADE
...
CREATE TABLE `campanha_itens` (
  ...
  `empresa_id` int NOT NULL,
  `tipo` enum('imagem','video','adsense','youtube','afiliado') NOT NULL,
  `ordem` int DEFAULT '0',
  ...
  `ad_width` smallint DEFAULT NULL,
  `ad_height` smallint DEFAULT NULL,
  `afiliado_link` varchar(500) DEFAULT NULL,
  `afiliado_imagem_url` varchar(500) DEFAULT NULL,
  `afiliado_descricao` text,
  `afiliado_preco` decimal(10,2) DEFAULT NULL,
  `afiliado_preco_original` decimal(10,2) DEFAULT NULL,
  `afiliado_destaques` text,
  `criado_em` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
...
CREATE TABLE `portal_campanhas` (
  ...
  KEY `idx_portal_campanhas_campanha` (`campanha_id`),
  CONSTRAINT `fk_portal_campanhas_portal` FOREIGN KEY (`portal_id`) REFERENCES `portais` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_portal_campanhas_campanha` FOREIGN KEY (`campanha_id`) REFERENCES `campanhas` (`id`) ON DELETE CASCADE
...
CREATE TABLE `portais` (
  ...
  `campanha_rotacao_ultima_id` int DEFAULT NULL,
...
CREATE TABLE `radius_users` (
  ...
  `criado_em` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_radius_users_username` (`username`),
...
INSERT INTO `admins` (`id`, `empresa_id`, `email`, `nome`, `role`, `password`, `created_at`) VALUES
(1, 1, 'admin@empresa.com', 'Super Admin', 'super_admin', '$2b$10$SV2foD9AePXojCpHKx.n9eEOtikIydo2Ngg1wXC6vpghAQHxrmTXO', NOW());

-- Vincular admin a empresa
INSERT INTO `admin_empresas` (`admin_id`, `empresa_id`, `role`, `criado_em`) VALUES
(1, 1, 'owner', NOW());

-- 8 portais padrao da empresa 1
INSERT INTO `portais` (`empresa_id`, `nome`, `slug`, `tipo`, `url_redirect`, `ativo`, `configuracoes`, `criado_em`) VALUES
(1, 'LGPD - Coleta de Dados', 'lgpd', 'lgpd', '/cadastro', 1, NULL, NOW()),
(1, 'Planos - Pagamento', 'planos', 'planos', '/planos-cliente', 1, '{"pagamento_pix_ativo":true,"pagamento_cartao_ativo":true,"pix_trial_enabled":true,"pix_trial_duracao_minutos":5}', NOW()),
(1, 'Cadastro de LEAD', 'lead', 'lead', '/lead', 1, NULL, NOW()),
(1, 'Cadastro de LEAD (Sem Internet)', 'lead-passivo', 'lead_passivo', '/lead-passivo', 1, NULL, NOW()),
(1, 'Acesso Wi-Fi', 'login', 'login', '/login-hotspot', 1, NULL, NOW()),
(1, 'Trial por Tempo', 'trial-tempo', 'trial_tempo', '/trial-tempo', 1, '{"trial_duracao_minutos":3,"trial_velocidade_down":2,"trial_velocidade_up":2}', NOW()),
(1, 'Campanha + Acesso', 'campanha-pre-acesso', 'campanha_pre_acesso', '/campanha-pre-acesso', 1, '{"campanha_duracao_segundos":30,"acesso_duracao_minutos":60,"acesso_velocidade_down":5,"acesso_velocidade_up":2}', NOW()),
(1, 'Reconexão - Renovar Acesso', 'reconexao', 'reconexao', '/reconectar', 1, NULL, NOW());
