...
exports.criarPortal = async (req, res) => {
  const {
    nome, slug, tipo, descricao, html_content, url_redirect,
...
exports.previewPortal = async (req, res) => {
  ...
  try {
    ...
    if (portal.url_redirect) {
      return res.redirect(portal.url_redirect + "?mac=AA:BB:CC:DD:EE:FF&ip=192.168.1.100&preview=1&empresa_id=" + req.empresa_id + "&portal_id=" + id);
    }
...
// Lista as campanhas atualmente vinculadas ao portal (task 012 — N:N via
// portal_campanhas, substitui o antigo campanha_ativa_id 1:1).
exports.listarCampanhasVinculadas = async (req, res) => {
  ...
  try {
    ...
    const [rows] = await db.execute(
      "SELECT campanha_id FROM portal_campanhas WHERE portal_id = ? ORDER BY campanha_id ASC",
      [portalId]
    );
    res.json({ campanha_ids: rows.map((r) => r.campanha_id) });
  } catch (err) {
    console.error("Erro ao listar campanhas vinculadas ao portal:", err);
    res.status(500).json({ message: "Erro ao listar campanhas vinculadas ao portal" });
  }
};

// Substitui a lista de campanhas concorrentes vinculadas ao portal (task 012).
// `campanha_ids`: array de ids (pode ser vazio, remove todos os vínculos).
// Zera o ponteiro de rotação — o conjunto elegível mudou, não faz sentido
// manter o "último servido" apontando pra um id que pode nem estar mais na lista.
exports.vincularCampanha = async (req, res) => {
  const portalId = parseInt(req.params.portalId, 10);
  const campanhaIds = Array.isArray(req.body.campanha_ids)
    ? [...new Set(req.body.campanha_ids.map((id) => parseInt(id, 10)).filter((id) => !isNaN(id)))]
    : [];

  const conn = await db.getConnection();
  try {
    const [[portal]] = await conn.query(
      "SELECT id FROM portais WHERE id = ? AND empresa_id = ?",
      [portalId, req.empresa_id]
    );
    if (!portal) {
    ...
    if (campanhaIds.length > 0) {
      const [validas] = await conn.query(
        "SELECT id FROM campanhas WHERE id IN (?) AND empresa_id = ?",
        [campanhaIds, req.empresa_id]
      );
      if (validas.length !== campanhaIds.length) {
        conn.release();
        return res.status(400).json({ message: "Uma ou mais campanhas não foram encontradas" });
      }
    }

    await conn.beginTransaction();
    await conn.execute("DELETE FROM portal_campanhas WHERE portal_id = ?", [portalId]);
    if (campanhaIds.length > 0) {
      const values = campanhaIds.map((cid) => [portalId, cid]);
      await conn.query("INSERT INTO portal_campanhas (portal_id, campanha_id) VALUES ?", [values]);
    }
    await conn.execute(
      "UPDATE portais SET campanha_rotacao_ultima_id = NULL WHERE id = ?",
      [portalId]
    );
    await conn.commit();

    res.json({ success: true, campanha_ids: campanhaIds });
  } catch (err) {
    await conn.rollback();
    console.error("Erro ao vincular campanhas ao portal:", err);
    res.status(500).json({ message: "Erro ao vincular campanhas ao portal" });
  } finally {
    conn.release();
  }
};
