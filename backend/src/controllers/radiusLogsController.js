...
exports.listarLogs = async (req, res) => {
  try {
    ...
    const per_page = Math.min(Math.max(parseInt(req.query.per_page || "12", 10), 1), 500);
    const offset = (page - 1) * per_page;

    // EXISTS em vez de INNER JOIN: imune a duplicatas residuais em radius_users
    const where = ["EXISTS (SELECT 1 FROM radius_users ru WHERE ru.username = ra.username AND ru.empresa_id = ?)"];
    ...
    const [rows] = await db.query(
      ...
      FROM radacct ra
      LEFT JOIN mikrotiks m ON (m.ip = ra.nasipaddress OR (m.vpn_ip IS NOT NULL AND m.vpn_ip = ra.nasipaddress))
