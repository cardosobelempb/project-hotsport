...
// Perfil da empresa (autoatendimento do próprio tenant - owner/manager)
exports.obterPerfilEmpresa = async (req, res) => {
  try {
    const [[empresa]] = await db.execute(
      'SELECT nome, logo_url, descricao FROM empresas WHERE id = ?',
      [req.empresa_id]
    );
    ...
    res.status(500).json({ message: "Erro ao obter perfil da empresa" });
  }
};

exports.atualizarPerfilEmpresa = async (req, res) => {
  try {
    const { nome, descricao } = req.body;
    if (!nome) return res.status(400).json({ message: "Nome é obrigatório" });

    await db.execute(
      'UPDATE empresas SET nome = ?, descricao = ? WHERE id = ?',
      [nome, descricao || null, req.empresa_id]
    );

    res.json({ message: "Perfil atualizado", nome, descricao: descricao || null });
  } catch (err) {
