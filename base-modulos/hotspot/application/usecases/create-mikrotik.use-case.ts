...
import { IpAddressVO } from "@/common/domain/values-objects/ip-address/ip-address.vo.js";
import { MacAddressVO } from "@/common/domain/values-objects/mac-address/mac-address.vo.js";
...
export class CreateMikrotikUseCase {
  ...
  async execute(
    ...
    const mikrotikExists = await this.mikrotikRepository.findById(
      input.macAddress,
    );
    ...
    const newMikrotik = MikrotikEntity.create({
      ...
      macAddress: MacAddressVO.create(input.macAddress),
